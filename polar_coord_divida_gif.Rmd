...
## Resgates da d?vida p?blica desde 2007
...
# (2) to excluindo essa coluna, pq n?o me interessa
# (3) a tabela original possui colunas para os totais dos anos. quando uso o "separate", ele n?o encontra o "/" e gera um NA para o "Ano". Aqui estou removendo essas linhas.
# (4) para o "Ano" ficar com 4 d?gitos e em formato num?rico.
# (5) porque em 2006 s? existem dados para novembro e dezembro.
```
...
O ideal seriam oito categorias, j? que as paletas de gradiente do `RColorBrewer` t?m 8 cores. Queria que a escala do eixo y tivesse os mesmos breaks da escala de cor.

```{r}
summary(resgates$Valor)
```

O m?ximo por enquanto ? 161 bilh?es. Vou considerar uma escala de 0 a 200 bi ent?o, com 8 n?veis de 25 bi de tamanho.
...
resgates_frag <- resgates # criando uma c?pia 
niveis_faixas <- NULL
...
# (1) dou nome ?s novas vari?veis...
# (2) aqui fa?o a fragmenta??o, preenchendo as colunas adequadas.
...
plot_equalizador <- ggplot(dados_plot_frag, aes(x = Mes, y = Valor_faixa, fill = Faixa)) +
  ...
  geom_text(aes(label = ifelse(Ano == 2018, 
                               format(round(Valor/1000,0), 
                                      big.mark = ".", 
                                      decimal.mark = ","),
                               NA),
                y = Valor + 15000,
...
animate(plot_din_frag + ease_aes('linear'), 
        bg = 'transparent', height = 488, width = 688, 
        renderer = gifski_renderer(loop = FALSE))

# anim_save("divida_polar.gif", animation = last_animation())
