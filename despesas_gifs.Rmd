...
Salvando o gif...
```{r}
anim_save("despesas2.gif", animation = last_animation())
```
...
slope_graph <- ggplot(dados_slope, aes(color = variacao < 0)) +
  geom_segment(
    aes(
      x = 0,
      xend = distancia,
      ...
      yend = `2017`), 
    lineend = "round")+
  geom_text(label="2008", x=0, y=(1.1*(valor_maximo)), hjust = "center", size=4.5, family = "Open Sans", color = 'grey20', fontface = 'bold') +
  geom_text(label="2017", x=distancia, y=(1.1*valor_maximo), hjust= "center", size=4.5, family = "Open Sans", color = 'grey20', fontface = 'bold') +
  geom_text(
  aes(x = 0 - distancia * 0.2,
      y = `2008`, # um vetor para ajustar pos
  ...
  geom_text(
    aes(x = distancia,
        y = `2017` * c(1, 0.9, 0.7, 1.1, 1, 1, 1, 1), # um vetor para ajustar pos
        label = paste(" ",
                      percent(`2017`), 
                      Categoria_reduzida,
                      sep="   "),
  ...
  scale_color_manual(values = c('TRUE' = '#1f476a', 'FALSE' = '#ba3917'), 
                     guide = 'none') +
  ylim(0, 1.1*valor_maximo) +
  xlim(0 - distancia * 0.2, distancia+distancia) +
  labs(
    x = NULL,
    y = NULL
  ) + tema_slope()
