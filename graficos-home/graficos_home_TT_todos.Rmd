...
graf_desp_para_gif <- graf_linhas_desp +
  ...
  geom_text(aes(x = ultimo_periodo + 50,
                ...
                y = ifelse(Periodo < ultimo_periodo,
                           Valor_12m_ipca,
                           Valor_12m_ipca + c(0, 15, -2, -5, -17, 35, -39, 35))), 
...
ggplot(gastos_prim_saude, aes(x = Periodo, color = tipo, y = valor)) +
  geom_line() +
  geom_text(aes(label = ifelse(Periodo == "2020-02-01", valor, NA))) +
  tema() +
  theme(legend.position = "none")
...
ggplot(gastos_saude_prim, aes(x = Periodo, y = relacao)) +
  ...
  geom_point(aes(y = ifelse(lubridate::month(Periodo) == 12, relacao, NA)),
             color = azul) +
  geom_text(aes(label = ifelse(lubridate::month(Periodo) == 12, percent(relacao, accuracy = .1), NA), y = relacao - 100),
                  family = "Merriweather Sans", size = 2, color = azul,
                  hjust = "center") +
  scale_y_continuous(labels = scales::percent, limits = c(0, NA)) +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  labs(x = NULL, y = NULL, title = "Despesas com saúde em relação ao gasto primário total", subtitle = "Valores acumulados em 12 meses, atualizados pelo IPCA", caption = "Fonte: RTN, Siafi") +
  theme_minimal() +
  theme(text = element_text(family = "Merriweather Sans"),
        plot.title = element_text(family = "Merriweather", color = "#1e1e1e",
                                  face = "bold", size = 12),
        ...
        panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
        axis.line = element_line(color = "#1e1e1e", size = 1),
        axis.ticks = element_line(size = 0.5))
```
