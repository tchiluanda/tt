...
grafico_linha_dpf <- ggplot(dpf, aes(x = Periodo, y = Valor_ipca, color = TRUE, group = 1)) +
  ...
  coord_cartesian(
    ylim = c(min(dpf$Valor_ipca), max(dpf$Valor_ipca)),
...
graf_desp_para_gif <- graf_linhas_desp +
  ...
  geom_text(aes(x = ultimo_periodo + 50,
                ...
                y = ifelse(Periodo < ultimo_periodo,
                           Valor_12m_ipca,
                           Valor_12m_ipca + c(0, 25, -40, 0, -21, 0, 0, 10))), 
...
graf_linhas_small <- ggplot(desp_12m_ipca_small, aes(x = Periodo, y = Valor_12m_ipca, group = classificador)) +
  ...
  scale_x_date(breaks = janeiros,
              #date_breaks = "1 years", 
               ...
               #date_labels = "%Y",
               expand = expand_scale(mult = c(0, .50))) +
  scale_color_manual(values = c("TRUE" = "firebrick", "FALSE" = "#efefef")) + 
  ...
  theme(legend.position = 'none') +
  theme(
    axis.text = element_text(size = 5, family = "Roboto"), # era 10
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
        axis.title.y = element_blank(),
    strip.text = element_text(hjust = 0, family = "Roboto"),
          axis.ticks = element_line(size = 0.25),
      axis.ticks.length = unit(.15, "cm")) +
  facet_wrap(~classificador2)

ggsave(plot = graf_linhas_small, filename = "small.png", h = 6, w = 10)
```
