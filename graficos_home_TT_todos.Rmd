...
graf_linhas_desp <- ggplot(dados_desp_plot, aes(x = Periodo, y = Valor_12m_ipca, group = classificador, color = classificador)) +
  ...
  geom_text(aes(x = max(dados_desp_plot$Periodo) + 50,
                ...
                y = ifelse(Periodo < ultimo_periodo,
                           Valor_12m_ipca,
                           Valor_12m_ipca + c(0, 15, 0, -23, 20, -10, 0))), 
  ...
  scale_x_date(breaks = janeiros,
              #date_breaks = "1 years", 
               labels = anos,
               #date_labels = "%Y",
               expand = expand_scale(mult = c(0, .67))) +
  scale_color_manual(values = brewer.pal(8, "Dark2")[-6]) +
  coord_cartesian(clip = 'off') +
  labs(
    y = NULL,
    x = NULL
...
# 
animate(gif_linhas_desp, nframes = nrow(dpf), height = 488, width = 668,
        renderer = gifski_renderer(loop = FALSE))
```
