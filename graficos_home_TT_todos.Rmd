...
graf_desp_para_gif <- graf_linhas_desp +
  ...
  geom_text(aes(x = ultimo_periodo + 50,
                ...
                y = ifelse(Periodo < ultimo_periodo,
                           Valor_12m_ipca,
                           Valor_12m_ipca + c(0, 19, 7, -15, -32, 33, -12, 30))), 
...
graf_linhas_desp + 
  ...
  geom_text(aes(x = ultimo_periodo + 50,
                ...
                y = ifelse(Periodo < ultimo_periodo,
                           NA,
                           Valor_12m_ipca + c(0, 15, 7, -15, -35, 30, -12, 30))), 
            hjust = 0, family = "Open Sans Condensed",
            size = 8)
...
desp_bar_estatico <- ggplot(desp_bar, 
                            aes(y = Valor_12m_ipca, 
                                x = rank, 
                                fill = classificador,
                                color = classificador)) +
  geom_bar(width = 0.7, stat = 'identity') +
  geom_text(aes(y = Valor_12m_ipca + 15, 
                label = valor_texto), # color = CoresEscuras
            position = position_dodge(1),
            size = 5, family = "Open Sans Condensed", vjust = 'center') +  
...
desp_bar_anim <- desp_bar_estatico + 
  transition_states(Periodo, transition_length = 1, state_length = 2) +
  labs(title = 'Despesas da União: últimos 12 meses até {meses[ultimo_mes]}',
...
\anim_save("despesas_bar.gif", animation = last_animation())
```
