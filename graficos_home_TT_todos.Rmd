...
graf_desp_para_gif <- graf_linhas_desp +
  ...
  geom_text(aes(x = ultimo_periodo + 50,
                label = paste(round(Valor_12m_ipca, 0), enc2native(classificador)),
...
desp_bar_estatico <- ggplot(desp_bar_cores, 
  ...
  geom_text(aes(y = Valor_12m_ipca + 15, #color = cores_escuras,
                label = valor_texto), 
...
desp_bar_estatico_anot <- desp_bar_estatico +
  ...
  geom_text(aes(label = ifelse(rank == 1, primeiro_ano, NA),
            ...
            check_overlap = TRUE,
            family = "Open Sans", size = 3, fontface = "italic")
...
anim_save("despesas_bar3.gif", animation = last_animation())
```
